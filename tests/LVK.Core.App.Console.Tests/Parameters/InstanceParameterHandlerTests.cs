...
public class InstanceParameterHandlerTests
{
    ...
    public void InjectParameters_PositionalArguments_AreAdded()
    {
        ...
        Assert.That(instance.Positional, Is.EqualTo(new[] {"a", "b", "c"}).AsCollection);
    }

    [Test]
    [TestCase(new[] { "-b", "true"}, true)]
    [TestCase(new[] { "--boolean", "true"}, true)]
    [TestCase(new[] { "-b", "false"}, false)]
    [TestCase(new[] { "--boolean", "false"}, false)]
    [TestCase(new[] { "-b" }, true)]
